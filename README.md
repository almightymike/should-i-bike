...
- Each rated window shows its best ride-out time for a two-hour ride based on the two consecutive hourly forecasts with the highest score, then the lowest combined wind, gust and rain burden. Both forecast hours must be at least 12°C, the rider's minimum comfort temperature. The full-window score stays conservative and does not include temperature. A window rated 1/5, or with fewer than two suitable full hours remaining, has no recommended start.
...
- Best and backup are the two highest-scoring complete upcoming windows above 1/5 with a suitable two-hour start. Within a score band, the lower combined wind, gust and rain burden wins. Weakest is the lowest ranked complete window. If no window has a suitable two-hour start, the page recommends no ride.
