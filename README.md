...
- A small OpenStreetMap preview shows the selected location's coordinates and updates when the location changes. It includes a link to the full map and visible OpenStreetMap attribution. The map iframe loads from OpenStreetMap in the visitor's browser, so OpenStreetMap receives a request when the thumbnail loads. It does not request device location or need an API key; the first-visit marker may be approximate.
